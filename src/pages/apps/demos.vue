...
const deleteUser = uid => {
  axios
    .delete(`${baseUrl}/ams_forms/demo/${uid}`)
    .then(response => {
      fetchContacts()
    })
    .catch(error => {
    ...
    <VCard>
      <VCardText class="d-flex flex-wrap gap-4">
        ...
        <!--
          ...
          <VBtn @click="router.push({name: 'add-training'})">
          <VIcon />
          Add Training
          </VBtn>
          </div>
        ...
        <template #item.actions="{ item }">
          ...
          >
            ...
            <VMenu activator="parent">
              <VList>
                <VListItem :to="{ name: 'view-demo', query: { demo : item.raw.uid,mail:item.raw.email } }">
                  <template #prepend>
